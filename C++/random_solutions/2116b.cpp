...
void solve_mine() {
    MOD = 998244353;
    auto n = read_int();
    auto p = read_int_list();
    auto q = read_int_list();
    vector<int> p_prefix_max(n + 1);
    vector<int> q_prefix_max(n + 1);
    q_prefix_max[0] = 0;
    p_prefix_max[0] = 0;
    for (int i = 1; i < n; i++) {
        if (q[q_prefix_max[i - 1]] < q[i]) {
            q_prefix_max[i] = i;
        } else {
            q_prefix_max[i] = q_prefix_max[i - 1];
        }
        if (p[p_prefix_max[i - 1]] < p[i]) {
            p_prefix_max[i] = i;
        } else {
            p_prefix_max[i] = p_prefix_max[i - 1];
        }
    }
    vector<ll> ans(n);
    // print_vector(p_prefix_max);
    // print_vector(q_prefix_max);
    for (int i = 0; i < n; i++) {
        pair<int, int> first_option = {p[p_prefix_max[i]], q[i - p_prefix_max[i]]};
        pair<int, int> second_option = {q[q_prefix_max[i]], p[i - q_prefix_max[i]]};
        auto [fst, snd] = max(first_option, second_option);
        ans[i] = mod_add(mod_exp(2, fst), mod_exp(2, snd));
    }
    for (ll el: ans) {
        cout << el << " ";
    }
...
void solve() {
    ...
    auto p = read_int_list();
    auto q = read_int_list();
    int p_max_idx = 0, q_max_idx = 0;
    for (int i = 0; i < n; i++) {
        if (p[p_max_idx] < p[i]) p_max_idx = i;
        if (q[q_max_idx] < q[i]) q_max_idx = i;
        auto first_option = make_pair(p[p_max_idx], q[i - p_max_idx]);
        auto second_option = make_pair(q[q_max_idx], p[i - q_max_idx]);
        auto [first, second] = max(first_option, second_option);
        cout << (mod_add(mod_exp(2, first), mod_exp(2, second))) << " ";
    }
    cout << "\n";
}
