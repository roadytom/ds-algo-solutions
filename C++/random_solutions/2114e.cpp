...
void dfs(int node, int parent, ll parent_minus, ll parent_plus) {
    answer[node] = values[node] + parent_minus;
    ll curr_minus = max(0ll, parent_plus - values[node]);
    ll curr_plus = max(0ll, parent_minus + values[node]);
    for (auto child: tree[node]) {
        if (child == parent) {
            continue;
        }
        dfs(child, node, curr_minus, curr_plus);
    }
}
