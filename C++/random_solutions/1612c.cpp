...
bool is_enough(ll mid, ll k, ll x) {
    ll stars = (k - 1 + mid) * (k - 1 - mid + 1) / 2;
    return stars <= x;
}

bool is_enough_top(ll mid, ll k, ll x) {
    ll stars = (1 + mid) * mid / 2;
    return stars <= x;
}
...
void solve() {
    ...
    while (left <= right) {
        ll mid = (left + right) / 2;
        if (mid >= k) {
            over = (k * (k + 1) / 2 + (k - 1) * k / 2 - (2 * k - 1 - mid) * (2 * k - mid) / 2) >= x;
        } else {
            over = ((mid + 1) * mid / 2) >= x;
        }
        if (over) {
    ...
    cout << (left > 2 * k - 1 ? 2 * k - 1 : left) << endl;
}
