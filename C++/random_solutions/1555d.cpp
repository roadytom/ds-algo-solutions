...
void solve() {
    int n, m;
    cin >> n >> m;
    vector<vector<int> > prefix_dp(6, vector<int>(n + 1, 0));
    string s;
    cin >> s;
    // preprocessing
    string t = "abc";
    ...
    do {
        for (int i = 0; i < n; i++) {
            prefix_dp[permutation_idx][i + 1] = prefix_dp[permutation_idx][i];
            if (s[i] != t[i % 3]) {
                prefix_dp[permutation_idx][i + 1]++;
            }
        }
        permutation_idx++;
    } while (next_permutation(t.begin(), t.end()));
    // print_vector(prefix_dp);
    while (m--) {
        int left, right;
        cin >> left >> right;
        left--;
        right--;
        int res = INF;
        for (auto &permutation_prefix: prefix_dp) {
            res = min(res, permutation_prefix[right + 1] - permutation_prefix[left]);
        }
        cout << res << endl;
    }
}
