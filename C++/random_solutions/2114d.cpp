...
/**
    n monsters
    can move one monster to ANY empty cell
    can choose a rectangle -> all monsters in the rectangle will be removed
                         -> pay 1 coin for EACH cell
...
struct MinMax {
    ...
    ll get_side_length_non_x(ll x) {
        pair<ll, ll> side = {min1, max1};
        if (x == min1) side.first = min2;
        if (x == max1) side.second = max2;
        return side.second - side.first + 1;
    }
};
...
void solve() {
    ...
    MinMax xs(points[0].first, points[1].first), ys(points[0].second, points[1].second);
    for (int i = 2; i < n; i++) {
        xs.insert(points[i].first);
        ys.insert(points[i].second);
    }
    ll area = xs.get_side_length_non_x(-1) * ys.get_side_length_non_x(-1);
    for (int i = 0; i < n; i++) {
        ll x = xs.get_side_length_non_x(points[i].first);
        ll y = ys.get_side_length_non_x(points[i].second);
        if (x * y == n - 1) {
            area = min(area, x * y + min(x, y));
        } else {
            area = min(area, x * y);
        }
    }
