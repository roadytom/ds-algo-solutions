...
int visited[30];
pair<int, int> memo[30][30];
pair<int, int> res;

class Solution {
public:
    void update_min_max(pair<int, int> &curr_max, int result) {
        curr_max.first = min(curr_max.first, result);
        curr_max.second = max(curr_max.second, result);
    }

    void backtrack_MINE_NOT_OPTIMAL(int n, int first_player, int second_player, int curr_round, int left_bound,
                                    int right_bound) {
        // if (curr_round > 100) {
        // return;
        // }
        // print_vector(vector<int>{curr_round, left_bound, right_bound});
        // for (int i = 0; i < n; i++) {
        // cout << visited[i] << " ";
        // }
        // cout << endl;
        ...
        while (left < right) {
            ...
            if (left == first_player and right == second_player) {
                update_min_max(res, curr_round);
                // print("here");
                return;
            }
            if (left == first_player or right == second_player or left == second_player or right == first_player) {
                if (left == first_player or left == second_player) {
                    visited[right] = 0;
                    backtrack_MINE_NOT_OPTIMAL(n, first_player, second_player, curr_round, left + 1, right - 1);
                    visited[right] = 1;
                } else {
                    visited[left] = 0;
                    backtrack_MINE_NOT_OPTIMAL(n, first_player, second_player, curr_round, left + 1, right - 1);
        ...
        backtrack_MINE_NOT_OPTIMAL(n, first_player, second_player, curr_round + 1, 0, n - 1);
    }

    vector<int> earliestAndLatest_MINE_NOT_OPTIMAL(int n, int first_player, int second_player) {
        ...
        res = pair<int, int>{INT_MAX, -1};
        backtrack_MINE_NOT_OPTIMAL(n, first_player - 1, second_player - 1, 1, 0, n - 1);
        return vector<int>{res.first, res.second};
    }

    pair<int, int> dp(int total_count, int between_count) {
        if (between_count == 0) {
            return {1, 1};
        }
    }
    ...
    vector<int> earliestAndLatest(int n, int first_player, int second_player) {
        ...
        pair<int, int> result = dp(n, second_player - first_player - 1);
    }
};
...
int main() {
    ...
    cout.tie(nullptr);
    Solution s;
    print_vector(s.earliestAndLatest(11, 2, 4));
    return 0;
}
