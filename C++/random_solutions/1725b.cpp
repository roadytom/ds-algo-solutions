...
/**
N players
can build team with any number of players
win -> sum(team p) > D
can make -> each player p = max(p)
find max num of wins
...
void solve() {
    ...
    int win_count = 0;
    while (l <= r) {
        ll curr_power = players[r];
        while (l < r and curr_power <= d) {
            curr_power += players[r];
            l++;
        }
        if (curr_power > d) {
            win_count++;
