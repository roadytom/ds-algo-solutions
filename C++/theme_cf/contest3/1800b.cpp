...
void solve() {
    ...
    for (auto &c: s) {
        int curr = values[get_idx(c)];
        int addend = get_value(c);
        if (abs(curr + addend) < abs(curr)) {
            res++;
        }
        values[get_idx(c)] = curr + addend;
    }
    for (int i = 0; i < 26; i++) {
        int val = abs(values[i]);
        int max_possible = val / 2;
        res += min(k, max_possible);
        k = max(0, k - max_possible);
        if (k == 0) {
            break;
        }
    }
