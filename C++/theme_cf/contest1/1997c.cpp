...
void solve() {
    ...
    for (auto &c: s) {
        if (get_value(c) > 0) close_count++;
        else if (get_value(c) < 0) open_count++;
    }
    int res = 0;
    for (int i = 0; i < s.size(); i++) {
        auto &c = s[i];
        if (get_value(c) > 0) res++;
        else if (get_value(c) < 0) res--;
        else {
            if (res == 0 || close_count <= 0) {
                s[i] = '(';
                res++;
                open_count--;
            } else {
    ...
    for (int i = 0; i < s.size(); i++) {
        if (get_value(s[i]) > 0) {
            ...
            int the_last_open = open_indices.top();
            open_indices.pop();
            res += i - the_last_open;
        }
    }
