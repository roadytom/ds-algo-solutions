...
void solve(int t) {
    ...
    signs.push_back(signs.back() + 1);
    while (q--) {
        ll d;
        cin >> d;

        auto it = lower_bound(all(signs), d + 1);
        ll idx = it - signs.begin() - 1;
        //
        // if (idx + 1 > k) {
        //     cout << t << " " << q << " here" << " " << idx << " " << d << endl;
        //     print_vector(signs);
        //     return;
        // }
        ll res = (times[idx + 1] - times[idx]) * (d - signs[idx]) / (signs[idx + 1] - signs[idx]) + times[idx];
        cout << res << " ";
    }
    cout << endl;
}
