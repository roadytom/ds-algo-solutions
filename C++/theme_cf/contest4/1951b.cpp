...
void solve() {
    ...
    // cout << endl;
    ll our_cow = scores[k];
    int first_larger_idx = -1;
    int second_larger_idx = n + 1;
    for (int i = 1; i <= n; i++) {
        if (first_larger_idx == -1 && our_cow < scores[i]) {
            first_larger_idx = i;
        } else if (our_cow < scores[i]) {
            assert(first_larger_idx != -1);
            second_larger_idx = i;
            break;
        }
    }
    // cout << first_larger_idx << " " << second_larger_idx << endl;
    if (first_larger_idx == -1) {
        cout << n - 1 << endl;
        return;
    }
    if (k < first_larger_idx) {
        cout << first_larger_idx - 2 << endl;
    } else {
        int first_option = first_larger_idx - 2;
        swap(first_larger_idx, k);
        first_larger_idx = min(first_larger_idx, second_larger_idx);
        int second_option = first_larger_idx - k - 1 + (k > 1);
