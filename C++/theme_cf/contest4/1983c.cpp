...
void print_ans(const vector<long long> &options, const vector<pair<int, int> > &pairs) {
    for (int i = 0; i < 3; i++) {
        for (int j = 0; j < options.size(); j++) {
            if (options[j] == i) {
                cout << pairs[j].first + 1 << " " << pairs[j].second + 1<< " ";
            }
        }
    }
    cout << endl;
...
void solve() {
    ...
    ll target = (tot + 2) / 3;
    vector<vector<ll> > values{a, b, c};
    set<int> indices{0, 1, 2};
    int l = 0;
    vector<ll> options{0, 1, 2};
    do {
        ll sum = 0;
        int idx = 0;
        while (idx < n && sum < target) {
            sum += values[options[0]][idx++];
        }
        if (idx >= n) {
            continue;
        }
        pair<int, int> f{0, idx - 1};
        sum = 0;
        while (idx < n && sum < target) {
            sum += values[options[1]][idx++];
        }
        if (idx >= n) {
            continue;
        }
        pair<int, int> s{f.second + 1, idx - 1};
        sum = 0;
        while (idx < n && sum < target) {
            sum += values[options[2]][idx++];
        }
        if (sum < target) {
            continue;
        }
        pair<int, int> t(s.second + 1, n - 1);
        print_ans(options, vector<pair<int, int> >{f, s, t});
        return;
