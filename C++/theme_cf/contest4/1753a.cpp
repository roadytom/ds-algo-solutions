...
void solve(int t) {
    ...
    auto put_if_non_zero = [&](int left, int right) {
        if (left > right) return;
        ans.emplace_back(left, right);
    };
    auto process_block = [&](int left, int right) {
        if (arr[left] == arr[right]) {
            if ((right - left - 1) % 2 == 0) {
                ans.emplace_back(left, right);
            } else {
                ans.emplace_back(left, left);
                put_if_non_zero(left + 1, right - 2);
                ans.emplace_back(right - 1, right);
            }
        } else {
            if ((right - left - 1) % 2 == 1) {
                ans.emplace_back(left, right);
                ...
                ans.emplace_back(left, left);
                put_if_non_zero(left + 1, right - 1);
                ans.emplace_back(right, right);
            }
        }
    };
    int first_non_zero = -1;
    int second_non_zero = -1;
    for (int i = 0; i < n; i++) {
        if (arr[i] != 0) {
            if (first_non_zero == -1) {
                first_non_zero = i;
            ...
            if (first_non_zero == -1 && second_non_zero == -1) {
                ans.emplace_back(i, i);
            }
        }
        if (first_non_zero != -1 && second_non_zero != -1) {
            process_block(first_non_zero, second_non_zero);
            first_non_zero = -1;
            second_non_zero = -1;
        }
    }
