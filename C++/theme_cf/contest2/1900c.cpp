...
void solve() {
    ...
    // for (int i = 1; i <= n; i++) {
    //     cout << "node " << i << " :" << tree[i].left << " " << tree[i].right << endl;
    // }
    auto dfs = [&](int node, auto &self) {
        // base case: we reached to leaf
        if (!tree[node].left && !tree[node].right) {
            return 0;
        }
        int left_cost = INT_MAX, right_cost = INT_MAX;
        if (tree[node].left) {
            left_cost = self(tree[node].left, self) + (s[node - 1] != 'L');
        }
        if (tree[node].right) {
            right_cost = self(tree[node].right, self) + (s[node - 1] != 'R');
        }
