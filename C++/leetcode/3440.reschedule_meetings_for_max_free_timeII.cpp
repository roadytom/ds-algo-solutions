...
class Solution {
public:
    int maxFreeTime(int event_time, const vector<int> &start_times, const vector<int> &end_times) {
        // map<int, int> free_times;
        vi free_times;
        int prev_end_time = 0;

        for (int i = 0; i < len(start_times); i++) {
            int start = start_times[i];
            int end = end_times[i];
            free_times.push_back(start - prev_end_time);
            prev_end_time = end;
        }
        free_times.push_back(event_time - prev_end_time);
        sort(all(free_times));
        prev_end_time = 0;
        int max_time = *max_element(all(free_times));
        for (int i = 0; i < len(start_times); i++) {
            int start = start_times[i];
            int end = end_times[i];
            int curr = end - start;
            int left = start - prev_end_time;
            int right = 0;
            if (i == len(start_times) - 1) {
                right = event_time - end;
            } else {
                right = start_times[i + 1] - end;
            }
            max_time = max(max_time, left + right);
            if (auto it = lower_bound(all(free_times), curr); free_times.end() - it >= (left >= curr) + (right >= curr) + 1) {
                max_time = max(max_time, left + right + curr);
            }
            prev_end_time = end;
        }
        return max_time;
...
int main() {
    ...
    cout.tie(nullptr);
    Solution s;
    print(s.maxFreeTime(77, {2, 51}, {35, 61}));
    // print(s.maxFreeTime(10, {0,7,9}, {1, 8, 10}));
    // print(s.maxFreeTime(5, {0,3,7,9}, {1,4,8,10}));
