...
/**
operation:
choose i 1 < i <= n -> make left elements arr[i]
cost = (i - 1) * arr[i]

choose i 1 <= i < n -> make right element arr[i]
cost = (n - i) * arr[i]
already arr[i] not changes the cost

MIN TOTAL COST ALL EQUAL
n = 5 * 10 ^ 5 -> nlog(n) at max
the smallest element would be a solution if every element is distinct
*/
void solve() {
    int N;
    cin >> N;
    ...
    while (right <= N + 1) {
        if (arr[right] != arr[left]) {
            ll curr_cost = arr[left] * (left - 1) + arr[left] * (N - right + 1);
            // cout << curr_cost << " " << left << " " << right << endl;
            cost = min(cost, curr_cost);
            left = right;
        }
        right++;
    }
    cout << (cost == LLONG_MAX ? 0 : cost) << endl;
}
