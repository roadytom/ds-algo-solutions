...
void solve() {
    ...
    for (int i = 2; i < len(is_prime); i++) {
        if (is_prime[i]) {
            for (int j = 2 * i; j < len(is_prime); j += i) {
                is_prime[j] = false;
            }
        }
    }
    vector<bool> visited(n + 1, false);
    // print_vector(is_prime);
    vector<int> ans(n + 1);
    for (int node = n; node >= 2; node--) {
        if (is_prime[node]) {
            vector<int> pairs;
            for (int multiple = node; multiple <= n; multiple += node) {
                if (!visited[multiple]) {
                    pairs.push_back(multiple);
                    visited[multiple] = true;
                }
            }
            // print_vector(pairs);
            for (int i = 0; i < len(pairs); i++) {
                ans[pairs[i]] = pairs[(i + 1) % len(pairs)];
            }
        }
    }
    ans[1] = 1;
