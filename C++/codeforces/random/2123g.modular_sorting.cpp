...
ll mod_inv(ll x) { return mod_exp(x, MOD - 2); }
...
template<typename T>
enable_if_t<!is_vector<T>::value> print_nested(const T &val, ostream &os) {
    os << val;
}

template<typename T>
enable_if_t<is_vector<T>::value> print_nested(const T &vec, ostream &os) {
    os << "[";
...
auto make_vector(size_t size, Args... args) {
    return vector<decltype(make_vector<T>(args...))>(size,
                                                     make_vector<T>(args...));
}
...
void solve() {
    ...
    vector<int> divisors;
    for (int divisor = 1; divisor * divisor <= m; divisor++) {
        if (m % divisor == 0) {
            divisors.push_back(divisor);
            if (divisor * divisor != m) {
                divisors.push_back(m / divisor);
            }
        }
    }
    for (int d: divisors) {
    ...
    while (q--) {
        ...
        if (type == 1) {
            ...
            for (auto &d: divisors) {
                if (i + 1 < n) {
                    answer[d] -= arr[i] % d > arr[i + 1] % d;
                }
                if (i - 1 >= 0) {
                    answer[d] -= arr[i - 1] % d > arr[i] % d;
                }
            }
            arr[i] = x;
            ...
            assert(type == 2, "wrong type");
            int k;
            cin >> k;
            int divisor = gcd(k, m);
            if (1ll * divisor * answer[divisor] >= m) {
                print("NO");
            } else {
                print("YES");
            }
