...
public:
    int mostBooked(int n, vector<vector<int> > &meetings) {
        priority_queue<pair<int, int>, vector<pair<int, int> >, greater<> > end_time_rooms;
        ...
        for (auto &meeting: meetings) {
            ...
            while (!end_time_rooms.empty() and end_time_rooms.top().first <= start) {
                unused.push(end_time_rooms.top().second);
                end_time_rooms.pop();
            }
            if (!unused.empty()) {
                int chosen_room = unused.top();
                counter[chosen_room]++;
                end_time_rooms.push({end, chosen_room});
                unused.pop();
            } else {
                auto [end_time, chosen_room] = end_time_rooms.top();
                counter[chosen_room]++;
                end_time_rooms.pop();
                end_time_rooms.push({end_time + end - start, chosen_room});
            }
        }
        int max_room_idx = 0;
        for (int i = 1; i < n; i++) {
            if (counter[max_room_idx] < counter[i]) {
                max_room_idx = i;
            }
        }
        return max_room_idx;
