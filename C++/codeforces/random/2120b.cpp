...
void solve() {
    ...
    int cnt = 0;
    for (int i = 0; i < n; i++) {
        int curr_x = coor[i].first;
        int curr_y = coor[i].second;
        int next_x = curr_x + d[i].first;
        int next_y = curr_y + d[i].second;
        // cout << curr_x << " " << curr_y << " " << next_x << " " << next_y << endl;
        //
        // if (curr_x == 1 and curr_y == 3) {
        //     print("here");
        //     cout << curr_x << " " << curr_y << " " << next_x << " " << next_y << endl;
        // }
        if (next_x - curr_x != 0) {
            int k = (next_y - curr_y) / (next_x - curr_x);
            int b = curr_y - k * curr_x;
            // cout << k << b << endl;
            for (auto &[kk, bb]: pairs) {
                if (kk == k and bb == b) {
                    cnt++;
                }
            }
