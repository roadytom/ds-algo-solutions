...
void solve() {
    int n;
    cin >> n;
    vector<int> assigned_universities(n);
    vector<int> skills(n);
    vector<vector<int> > universities(n);
    rep(i, 0, n) {
        int u;
        cin >> u;
        u--;
        assigned_universities[i] = u;
    }
    rep(i, 0, n) {
        cin >> skills[i];
    }
    rep(student, 0, n) {
        universities[assigned_universities[student]].push_back(skills[student]);
    }
    rep(university, 0, n) {
        sort(all(universities[university]), greater<>());
    }
    vector<vector<ll> > u_prefix_sums(n, vector<ll>(1, 0));
    rep(university, 0, n) {
        auto &university_skills = universities[university];
        for (auto &skill: university_skills) {
            u_prefix_sums[university].push_back(u_prefix_sums[university].back() + skill);
        }
    }
    vector<ll> ans(n);
    rep(uni, 0, n) {
        for (int k = 1; k <= universities[uni].size(); k++) {
            auto &prefix_sum = u_prefix_sums[uni];
            int size = (int) prefix_sum.size() - 1;
            int can_attend = size - size % k;
            ans[k - 1] += prefix_sum[can_attend];
        }
    }
    for (auto &x: ans) {
        cout << x << " ";
