...
void solve() {
    ...
    ll prev_normal = arr[0], prev_reversed = -arr[0];
    for (int i = 1; i < n; i++) {
        auto &it = arr[i];
        // reversed
        ll curr_reversed = max(prev_normal - it, prev_reversed + it);
        // normal
        ll curr_normal = max(prev_normal + it, prev_reversed - it);
        prev_reversed = curr_reversed;
        prev_normal = curr_normal;
    }
    cout << prev_normal << endl;
}
