...
void solve() {
    ...
    vector<int> dp(n + 1);
    dp[1] = 1;
    function<void(int, int, int)> dfs = [&](int node, int edge_idx, int parent) {
        for (auto &[child, child_edge_idx]: tree[node]) {
            if (child == parent) continue;
            if (edge_idx < child_edge_idx) {
                dp[child] = dp[node];
            } else {
                dp[child] = dp[node] + 1;
            }
            dfs(child, child_edge_idx, node);
        }
    };
    dfs(1, -1, -1);
