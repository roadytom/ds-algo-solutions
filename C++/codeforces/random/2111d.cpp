...
void solve() {
    int groups_count, classrooms_count;
    cin >> groups_count >> classrooms_count;
    vector<ll> classrooms(classrooms_count);
    for (int i = 0; i < classrooms_count; i++) {
        cin >> classrooms[i];
    }
    sort(classrooms.begin(), classrooms.end());
    int top_idx = 0, bottom_idx = classrooms_count - 1;
    // ll cnt = 0;
    for (int i = 0; i < groups_count; i++) {
        vector<int> temp{top_idx, bottom_idx};
        int k = 0;
        // ll prev = classrooms[temp[k]];
        for (int j = 0; j < 6; j++) {
            // cnt += abs(classrooms[temp[k]] / 100 - prev / 100);
            cout << classrooms[temp[k]] << " ";
            // prev = classrooms[temp[k]];
            k = (k + 1) % 2;
        }
        cout << endl;
        if (i + 1 < groups_count) {
