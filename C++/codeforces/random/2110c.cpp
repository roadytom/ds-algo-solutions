...
void solve() {
    int n;
    cin >> n;
    vector<int> flight_program(n);
    vector<pair<int, int> > obstacles(n);
    for (int i = 0; i < n; i++) {
        cin >> flight_program[i];
    }
    for (auto &[fst, snd]: obstacles) {
        cin >> fst >> snd;
    }
    int curr = 0;
    queue<int> buffer_indices;
    for (int i = 0; i < n; i++) {
        auto &[left, right] = obstacles[i];
        if (flight_program[i] >= 0) {
            curr += flight_program[i];
        } else {
            flight_program[i] = 0;
            buffer_indices.push(i);
        }
        if (curr <= right and curr + buffer_indices.size() >= left) {
            while (curr < left) {
                flight_program[buffer_indices.front()] = 1;
                buffer_indices.pop();
                curr++;
            }
            while (curr + buffer_indices.size() > right) {
                buffer_indices.pop();
            }
        } else {
            print(-1);
