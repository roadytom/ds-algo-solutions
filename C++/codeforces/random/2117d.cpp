...
void solve_mine() {
    ...
    ll b = -1;
    for (int i = 1; i < n; i++) {
        ll numerator = arr[i] * (i + 1) - arr[i + 1] * i;
        ll denom = n + 1;
        if (numerator % denom != 0) {
            print("NO");
            return;
        }
        ll curr_b = numerator / denom;
        if ((arr[i] - curr_b * (n - i + 1)) / i < 0 or curr_b < 0) {
            print("NO");
            return;
        }
        if (b != -1 and b != curr_b) {
...
void solve() {
    ...
    ll smallest = diff < 0 ? arr[n] : arr[1];
    diff = abs(diff);
    if (smallest - diff < 0 or (smallest - diff) % (n + 1) != 0) {
        print("NO");
        return;
    }
    print("YES");
