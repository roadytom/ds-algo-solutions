...
void solve() {
    ...
    for (int i = 0; i < n; i++) {
        int l, r;
        cin >> l >> r;
        segments.emplace_back(pair<int, int>{l, r}, i);
    }
    sort(all(segments));
    int max_end = segments[0].first.second;
    vector<int> res(n, 2);
    bool found = false;
    for (int i = 0; i < n; i++) {
        if (segments[i].first.first > max_end) {
            found = true;
            break;
        }
        res[segments[i].second] = 1;
        max_end = max(max_end, segments[i].first.second);
    }
    if (found) {
        for (int i = 0; i < n; i++) {
            cout << res[i] << " ";
