...
void solve() {
    ...
    ll min_prime_sum = 0ll;
    ll sum = 0;
    sort(rall(arr));
    int max_possible_n = 0;
    for (int i = 2; i <= MAXN; i++) {
        if (!is_prime[i]) {
            sum += arr[max_possible_n];
            min_prime_sum += i;
            if (min_prime_sum > sum) {
                break;
            }
            max_possible_n++;
            if (max_possible_n >= len(arr)) {
                break;
            }
        }
    }
    // cout << max_possible_n << " " << sum << " " << min_prime_sum << endl;
    cout << max(0, len(arr) - max_possible_n) << endl;
}
