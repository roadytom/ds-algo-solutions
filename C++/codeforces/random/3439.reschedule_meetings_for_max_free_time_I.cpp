...
#define range(l, r) for (int i = l; i < r; i++)
#define rrange(r, l) for (int i = r; i > l; i--)
#define ranges(l, r, step) for (int i = l; i < r; i += step)
#define rranges(r, l, step) for (int i = r; i > l; i -= step)
...
class Solution {
public:
    int maxFreeTime(int event_time, int k, const vector<int> &start_time, const vector<int> &end_time) {
        vi breaks;
        int prev_end = 0;
        range(0, len(start_time)) {
            int start = start_time[i];
            int end = end_time[i];
            breaks.push_back(start - prev_end);
            prev_end = end;
        }
        breaks.push_back(event_time - prev_end);
        int left = 0, right = 0;
        int window_sum = 0;
        int max_window_sum = -1;
        while (right < len(breaks)) {
            window_sum += breaks[right];
            max_window_sum = max(max_window_sum, window_sum);
            if (right - left == k) {
                // print(window_sum);
                window_sum -= breaks[left];
                left += 1;
            }
            right += 1;
        }
