...
void solve() {
    ...
    int last_wrong_position = n;
    while (last_wrong_position >= 1 and arr[last_wrong_position - 1] == last_wrong_position) {
        last_wrong_position -= 1;
    }
    if (last_wrong_position == 0) {
        cout << 1 << endl;
        return;
    }
    double ans = 1;
    for (auto [r, p]: experiments) {
        if (r < last_wrong_position) {
            continue;
        }
        ans *= (1 - p);
    }
