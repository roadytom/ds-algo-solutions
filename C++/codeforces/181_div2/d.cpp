...
void solve() {
    int n, m;
    cin >> n >> m;
    vector<vector<pair<ll, ll> > > cell_to_segments(m + 1);
    ll all_prod = 1;
    for (int i = 0; i < n; i++) {
        ll l, r, x, y;
        cin >> l >> r >> x >> y;
        ...
        r--;
        ll y_inv = mod_inv(y);
        ll prob = mod_mul(x, y_inv);
        ll one_min_prob = mod_sub(1, prob);
        all_prod = mod_mul(all_prod, one_min_prob);
        ll weight = mod_mul(prob, mod_inv(one_min_prob));
        cell_to_segments[l].emplace_back(r, weight);
    }
    vector<ll> ans(m + 1, -1);
    auto dp = [&](int cell, auto &self) {
        if (cell >= m) {
            return (cell == m ? all_prod : 0LL);
        }
        if (ans[cell] != -1) {
            return ans[cell];
        }
        ll prop = 0;
        auto &it = cell_to_segments[cell];
        for (auto &[r, weight]: it) {
            prop = mod_add(prop, mod_mul(weight, self(r + 1, self)));
        }
        ans[cell] = prop;
        return prop;
    };
