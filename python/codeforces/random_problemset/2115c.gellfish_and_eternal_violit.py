...
def solve():
    ...
    def dp(round_idx, hps):
        if round_idx >= m:
            return 1, all(hp == 1 for hp in hps)
        total1, correct1 = dp(round_idx + 1, hps)
        total2, correct2 = dp(round_idx + 1, [x - 1 for x in hps])
        for monster_idx in range(n):
            hps[monster_idx] -= 1
            total3, correct3 = dp(round_idx + 1, hps)
            hps[monster_idx] += 1
