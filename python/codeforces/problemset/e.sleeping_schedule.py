...
def main():
    n, h, l, r = read_int_list()
    arr = read_int_list()

    def will_do_good_sleep(_time):
        return 1 if l <= _time <= r else 0
    ...
    for day in range(n - 1, -1, -1):
        for time in range(h):
            awake1 = (time + arr[day]) % h
            awake2 = (time + arr[day] - 1) % h
            good1 = will_do_good_sleep(awake1)
            good2 = will_do_good_sleep(awake2)
            dp[day][time] = max(dp[day + 1][awake1] + good1, dp[day + 1][awake2] + good2)
