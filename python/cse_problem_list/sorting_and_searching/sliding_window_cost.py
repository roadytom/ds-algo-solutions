...
class SegmentTree:
    ...
    def build_tree(self, arr):
        def do_build_tree(tree_idx, seg_left, seg_right):
            if seg_left == seg_right:
                self.tree[tree_idx] = arr[seg_left]
            mid = (seg_left + seg_right) // 2
            do_build_tree(2 * tree_idx + 1, seg_left, mid)
            do_build_tree(2 * tree_idx + 2, mid + 1, seg_right)
...
def main():
    ...
    while right < n:
        ...
        if right - left + 1 == k:
            ...
            # print(median_idx)
            res.append(idx_to_val[median_idx])
